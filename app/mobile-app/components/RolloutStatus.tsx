  ...
  return (
    <section className="relative w-full overflow-hidden bg-gray-50 py-20 dark:bg-slate-950">
      ...
      <div className="relative mx-auto max-w-[1440px] px-6 sm:px-10 lg:px-16">
        ...
        {/* Status Cards */}
        <div className="mx-auto mt-16 grid max-w-[920px] grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {statusCards.map((card) => (
            <div
              key={card.title}
              className="flex min-h-[96px] flex-col items-center justify-center rounded-xl border border-slate-200 bg-white px-3 py-4 text-center shadow-[0px_6px_18px_rgba(14,31,61,0.05)] dark:border-slate-800 dark:bg-slate-900 dark:shadow-[0px_6px_18px_rgba(0,0,0,0.18)]"
            >
              <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-500 dark:text-slate-500">
                {card.title}
              </p>

              <p className="mt-2 text-xs font-semibold leading-4 text-slate-800 dark:text-slate-200">
                {card.value}
              </p>
            </div>
          ))}
        ...
        {/* App Store Cards */}
        <div className="mx-auto mt-12 grid max-w-[600px] grid-cols-1 gap-5 sm:grid-cols-2">
          {stores.map((store) => (
            <div
              key={store.name}
              className="flex h-32 flex-col items-center justify-center rounded-2xl border border-gray-300 bg-white dark:border-slate-700 dark:bg-slate-900"
            >
              ...
              <span className="mt-3 rounded-full bg-gray-100 px-4 py-1 text-[10px] font-bold uppercase tracking-wide text-gray-500 dark:bg-slate-800 dark:text-slate-400">
                Coming soon
              </span>
            </div>
          ))}
        ...
        <div className="mt-12 flex justify-center">
          <button className="h-11 rounded-[10px] bg-teal-600 px-7 text-sm font-semibold text-white shadow-[0px_6px_16px_rgba(16,162,141,0.28)] transition hover:bg-teal-700 dark:bg-teal-500 dark:hover:bg-teal-400">
