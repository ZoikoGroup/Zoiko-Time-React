...
export type AssurancePracticeCardProps = {
  title: string;
  status: { label: string; tone: AssuranceTone };
  objective: ReactNode;
  rows: { label: string; value: ReactNode }[];
  limitation: ReactNode;
};

export default function AssurancePracticeCard({
  title,
  status,
  objective,
  rows,
  limitation,
}: AssurancePracticeCardProps) {
  return (
    <article className="flex flex-col overflow-hidden rounded-2xl border border-stone-200 bg-white dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="flex flex-col gap-3 px-6 pb-4 pt-6 sm:flex-row sm:items-start sm:justify-between">
        ...
        <AssuranceStatusPill label={status.label} tone={status.tone} />
