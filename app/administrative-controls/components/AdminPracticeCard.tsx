...
export type AdminPracticeCardProps = {
  title: string;
  status: { label: string; tone: AdminTone };
  objective: ReactNode;
  rows: { label: string; value: ReactNode }[];
  /** Defaults to "Limitations" — the break-glass card uses "Hard limits". */
...
export default function AdminPracticeCard({
  ...
  objective,
  rows,
  limitationLabel = "Limitations:",
  limitation,
}: AdminPracticeCardProps) {
  return (
    <article className="flex flex-col overflow-hidden rounded-2xl border border-stone-200 bg-white dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="border-t border-dashed border-amber-200 bg-amber-50/50 px-6 py-4 dark:border-amber-900/40 dark:bg-amber-950/10">
        ...
        <p className="text-xs font-normal leading-5 text-amber-900 dark:text-amber-200/80">
          <span className="font-bold">{limitationLabel}</span> {limitation}
        </p>
